...
for i in raw_data:
    if type(i[filed_names[0]]) is not int:
        total_errors += 1
        bus_ids += 1
    if type(i[filed_names[1]]) is not int:
        total_errors += 1
        stop_ids += 1
    if type(i[filed_names[2]]) is not str or i[filed_names[2]] == '':
        total_errors += 1
        stop_names += 1
    if type(i[filed_names[3]]) is not int:
        total_errors += 1
        next_stops += 1
    if type(i[filed_names[4]]) is not str or len(i[filed_names[4]]) > 1:
        total_errors += 1
        stop_types += 1
    if type(i[filed_names[5]]) is not str or len(i[filed_names[5]]) != 5:
        total_errors += 1
        a_times += 1
