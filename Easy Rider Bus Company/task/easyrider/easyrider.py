...
def check_time_format(data):
    if type(data) is not str:
        ...
        regs = r'[0-2][0-9]:[0-5][0-9]'
        if not re.match(regs, data):
            return True


def check_stop_name(data):
    if type(data) is not str:
        return True
    elif type(data) is str:
        regs = r"\b([A-Z][a-z]*\s)+(?:Road|Avenue|Boulevard|Street)$\b"
        if not re.match(regs, data):
            return True
...
for i in raw_data:
    if check_integer_type(i[filed_names[0]]):
        bus_ids += 1
        total_errors += 1
    if check_integer_type(i[filed_names[1]]):
        stop_ids += 1
        total_errors += 1
    if check_stop_name(i[filed_names[2]]):
        stop_names += 1
        total_errors += 1
    if check_integer_type(i[filed_names[3]]):
        next_stops += 1
        total_errors += 1
